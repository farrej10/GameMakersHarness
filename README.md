...
- Commands below use `npm` and `npx` on macOS/Linux. If Windows PowerShell blocks its `.ps1` launchers, use `npm.cmd` and `npx.cmd` instead.
...
```sh
npm run game:generate -- --run <run-id> --demo-fault logic-victory
```
...
Agent outputs and errors are collapsed by default. Selecting an agent opens only that agent's accepted artifact, raw attempts, and validation errors. The optional agent-instructions panel lets users add separate guidance for logic, level, art, repair, and each individual sprite; this guidance is appended to the protected role prompt and saved with the run. Existing runs can be reopened from the run selector. Stopped, paused, and verified runs expose a targeted regeneration action inside the instructions panel. It creates a linked run that reruns failed or changed roles and records unchanged outputs as reused, leaving the source run and verified build intact. Changing one sprite instruction requests only that sprite and preserves the other rendered images; changing the all-art instruction requests all four. Spinners identify specification generation, worker execution, artifact loading, uploads, builds, and verification while they are active.
