...
function runSession(runRoot: string): Record<string, unknown> {
  ...
  return {
    ...
    agentInstructions: readAgentInstructions(runRoot),
  };
}

function retryStoppedRun(
  runsRoot: string,
  sourceRunId: string,
  ...
  if (sourceStatus?.state !== 'stopped' && sourceStatus?.state !== 'reviewing' && sourceStatus?.state !== 'verified') {
    throw new Error('Only a stopped, reviewing, or verified run can create an iteration.');
  }
...
<details id="agent-instructions" class="card"><summary><strong>Agent instructions (optional)</strong></summary><p class="help">These steer implementation but cannot override the approved specification. To change when the game ends, create a new specification and set its objective to <code>custom</code> with an explicit <code>winCondition</code>.</p><label for="instruction-logic">Logic agent</label><textarea id="instruction-logic" rows="4" maxlength="2000" placeholder="Example: Implement the approved custom win condition exactly and do not add prose."></textarea><label for="instruction-level">Level agent</label><textarea id="instruction-level" rows="4" maxlength="2000" placeholder="Example: Put the exit in the lower-right and keep the center open."></textarea><label for="instruction-art">All art (changing this regenerates all four sprites)</label><textarea id="instruction-art" rows="4" maxlength="2000" placeholder="Example: Use a chunky top-down arcade style with strong silhouettes."></textarea><details id="sprite-instructions"><summary>Individual sprite instructions</summary><label for="instruction-art-player">Player sprite</label><textarea id="instruction-art-player" rows="3" maxlength="2000"></textarea><label for="instruction-art-collectible">Collectible sprite</label><textarea id="instruction-art-collectible" rows="3" maxlength="2000"></textarea><label for="instruction-art-enemy">Enemy sprite</label><textarea id="instruction-art-enemy" rows="3" maxlength="2000"></textarea><label for="instruction-art-exit">Exit sprite</label><textarea id="instruction-art-exit" rows="3" maxlength="2000"></textarea></details><label for="instruction-repair">Repair agent</label><textarea id="instruction-repair" rows="4" maxlength="2000" placeholder="Example: Make the smallest valid replacement that addresses the failing check."></textarea><br><button id="apply-instructions" type="button" hidden>Regenerate agents with changed instructions</button></details>
...
<details id="outputs" class="card" hidden><summary><strong>Agent outputs and errors</strong></summary><div id="output-list"></div><button id="retry" hidden>Run autonomous repair · Retry failed run</button></details>
...
function setBusy(message,busy=true,reportRunId=null){status.replaceChildren();status.append(document.createTextNode(message));if(reportRunId){const separator=document.createTextNode(' · ');const reportLink=document.createElement('a');reportLink.href='/report/'+reportRunId;reportLink.textContent='View execution report';status.append(separator,reportLink)}spinner.hidden=!busy}
...
function focusInstruction(key){const field=q('#'+instructionFields[key]);if(!field)return;for(let parent=field.parentElement;parent;parent=parent.parentElement)if(parent.tagName==='DETAILS')parent.open=true;field.focus({preventScroll:true});field.scrollIntoView({behavior:'smooth',block:'center'})}
...
async function poll(){try{const response=await fetch('/api/run/'+current.runId),run=await response.json();if(!response.ok)throw new Error(run.error);current.state=run.state;renderProgress(run);const active=['generating','integrating','verifying','repairing'].includes(run.state),canIterate=['reviewing','verified','stopped'].includes(run.state);q('#apply-instructions').hidden=!canIterate;q('#apply-instructions').textContent=run.state==='stopped'?'Retry failed and changed agents':'Regenerate agents with changed instructions';setBusy(run.state+(run.message?' · '+shortMessage(run.message):''),active);if(run.state==='reviewing'){clearInterval(pollTimer);await Promise.all([loadReview(),loadOutputs()])}if(run.state==='verified'||run.state==='stopped'){clearInterval(pollTimer);q('#report').href='/report/'+current.runId;q('#report').hidden=false;await loadOutputs();q('#retry').hidden=run.state!=='stopped';if(run.state==='verified'){setBusy('Starting verified game preview…');const play=await call('/api/play',{runId:current.runId});q('#game').src=play.origin;q('#game').hidden=false;setBusy('Verified.',false)}else setBusy('Stopped · '+shortMessage(run.message||'Inspect the agent output, then retry.'),false,current.runId)}}catch(e){clearInterval(pollTimer);setBusy(e.message,false)}}
...
async function startInstructionIteration(){try{setBusy('Planning targeted agent iteration…');const endpoint=current.state==='stopped'?'/api/retry':'/api/iterate',iterated=await call(endpoint,{runId:current.runId,pauseForReview:true,agentInstructions:gatherInstructions()});current={...iterated,state:'generating'};loadInstructions(iterated.agentInstructions);renderSpec(current.spec);q('#review').hidden=true;q('#outputs').hidden=true;q('#outputs').open=false;q('#apply-instructions').hidden=true;q('#report').hidden=true;q('#game').hidden=true;await refreshRuns();q('#runs').value=current.runId;setBusy('Rerunning '+iterated.plan.rerunRoles.join(', ')+'; reusing unchanged outputs…');beginPoll()}catch(e){setBusy(e.message,false)}}
q('#regenerate-instructed').addEventListener('click',startInstructionIteration);
q('#apply-instructions').addEventListener('click',startInstructionIteration);
q('#retry').addEventListener('click',async()=>{try{setBusy('Starting autonomous repair…');const retried=await call('/api/retry',{runId:current.runId,pauseForReview:q('#review-gate').checked,agentInstructions:gatherInstructions()});current=retried;loadInstructions(retried.agentInstructions);renderSpec(current.spec);q('#retry').hidden=true;q('#outputs').hidden=true;q('#outputs').open=false;q('#report').hidden=true;q('#game').hidden=true;await refreshRuns();q('#runs').value=current.runId;beginPoll()}catch(e){setBusy(e.message,false)}});
    ...
    try {
      ...
      if (url.pathname === '/api/generate') {
        ...
        if (body.agentInstructions !== undefined) {
          writeAgentInstructions(safeRunRoot(runsRoot, body.runId), body.agentInstructions);
        }
        void actions
          .generate(body.runId, {
      ...
      if (url.pathname === '/api/retry') {
        ...
        const retried = retryStoppedRun(runsRoot, body.runId, body.agentInstructions);
