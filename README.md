...
Agent outputs and errors are collapsed by default. Selecting an agent opens only that agent's accepted artifact, raw attempts, and validation errors. The optional agent-instructions panel lets users add separate guidance for logic, level, art, and repair; this guidance is appended to the protected role prompt, saved with the run, and carried into retries. Existing runs can be reopened from the run selector. A stopped run exposes **Retry failed run**, which creates a linked run from the same approved specification and leaves the failed evidence unchanged. Spinners identify specification generation, worker execution, artifact loading, uploads, builds, and verification while they are active.
